...
# Yocto Variables
YOCTO_IMAGE ?=          "${YOCTO_DEFAULT_IMAGE}-${MACHINE}.tar.xz"
...
do_recovery_sdcard() {
    ...
    install -m 0644 ${DEPLOY_DIR_IMAGE}/wificfg ${IMAGE_ROOTFS}/opt/images/Yocto/boot

    # Install Yocto image for eMMC
    if [ ! -f ${YOCTO_IMAGE_PATH} ]; then
        bberror "Could not find Yocto image ${YOCTO_IMAGE_PATH}"
    fi
    install -m 0644 ${YOCTO_IMAGE_PATH} ${IMAGE_ROOTFS}/opt/images/Yocto/rootfs.tar.xz
}
