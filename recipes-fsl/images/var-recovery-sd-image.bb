...
#       SD_IMAGE="recipes-fsl/images/fsl-image-gui.bb"
#
#   YOCTO_DEFAULT_IMAGE:
...
DESCRIPTION = "Variscite recovery SD card image suitable for installing fsl-image-gui on eMMC"
LICENSE = "MIT"
...
# Yocto Variables
YOCTO_IMAGE ?=          "${YOCTO_DEFAULT_IMAGE}-${MACHINE}.tar.gz"
YOCTO_IMAGE_PATH ?=     "${DEPLOY_DIR_IMAGE}/${YOCTO_IMAGE}"
YOCTO_SCRIPTS_PATH ?=   "${THISDIR}/../../scripts/var_mk_yocto_sdcard/variscite_scripts"
YOCTO_INSTALL_SCRIPT ?= "mx8_install_yocto.sh"

do_recovery_sdcard() {
    # Install scripts for eMMC/Nand installation
    install -d ${D}/${bindir}
    install -m 755 ${YOCTO_SCRIPTS_PATH}/echos.sh ${IMAGE_ROOTFS}/${bindir}/
    install -m 755 ${YOCTO_SCRIPTS_PATH}/${YOCTO_INSTALL_SCRIPT} \
                    ${IMAGE_ROOTFS}/${bindir}/install_yocto.sh

    # Install imx-boot image for eMMC
    # Some SoCs have multiple IMXBOOT_TARGETS. Use the first one for the default
    # imx-boot-sd.bin image:
    IMXBOOT_TARGET=$(echo ${IMXBOOT_TARGETS} | awk '{print $1;}')
    install -d ${IMAGE_ROOTFS}/opt/images/Yocto
    install -m 0644 ${DEPLOY_DIR_IMAGE}/imx-boot-${MACHINE}-sd.bin-* ${IMAGE_ROOTFS}/opt/images/Yocto/
    install -m 0644 ${DEPLOY_DIR_IMAGE}/imx-boot-${MACHINE}-sd.bin-${IMXBOOT_TARGET} ${IMAGE_ROOTFS}/opt/images/Yocto/imx-boot-sd.bin

    # Install Yocto image for eMMC
    if [ ! -f ${YOCTO_IMAGE_PATH} ]; then
        bberror "Could not find Yocto image ${YOCTO_IMAGE_PATH}"
    fi
    install -m 0644 ${YOCTO_IMAGE_PATH} ${IMAGE_ROOTFS}/opt/images/Yocto/rootfs.tar.gz
}
